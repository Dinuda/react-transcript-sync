...
export default function loadTranscriptTracer(
  options: any = null,
  vttText: string
): void {
  ...
  for (let i = 0; i < ttMediaPlayers.length; i++) {
    ...
    mediaPlayer.addEventListener("play", function (e) {
      if (ttActivePlayer != e.currentTarget) {
        ...
        // @ts-ignore - TypeScript doesn't recognize dataset properties
        if (ttCurrentTranscript) clearHighlightedWords(ttCurrentTranscript);
        // @ts-ignore - TypeScript doesn't recognize cript d properties
...
function convertTicksToSeconds(ticks: any) {
  return ticks / 10_000_000; // 1 tick = 100 nanoseconds = 10^-7 seconds
}
